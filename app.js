  ...
  state.legs.forEach((leg, i) => {
    ...
    row.innerHTML = `
      ...
      <div class="field city-field">
        <label>City</label>
        <input type="text" placeholder="e.g. London" value="${escape(leg.city)}" data-i="${i}" data-k="city" autocomplete="off">
      ...
      <button class="remove-leg" data-i="${i}" title="Remove leg">×</button>
    `;
  ...
  geocodeTimer = setTimeout(async () => {
    try {
      ...
      box.innerHTML = data.results.map(r => `
        <div class="suggestion" data-lat="${r.latitude}" data-lon="${r.longitude}" data-name="${escape(r.name)}">
          <div>${r.name}</div>
          <div class="suggestion-sub">${[r.admin1, r.country].filter(Boolean).join(', ')}</div>
        </div>
      `).join('');
      box.hidden = false;
...
function renderDayCard(day) {
  ...
  if (!e) {
    card.innerHTML = `<div class="dow">${dowFor(day.date)}</div>
      <div class="date">${shortDate(day.date)}</div>
      <div class="city">${day.city}</div>
      <div style="margin:30px 0; color: var(--text-faint); font-size: 12px;">No data</div>`;
    return card;
  }
  ...
  card.innerHTML = `
    <div class="conf-dot-day conf-${conf}" title="Model agreement: ${conf === 'high' ? 'high' : conf === 'med' ? 'some spread' : 'models disagree'} (Δ${spread})"></div>
    <div class="dow">${dowFor(day.date)}</div>
    <div class="date">${shortDate(day.date)}</div>
    <div class="city" title="${day.city}">${day.city}</div>
